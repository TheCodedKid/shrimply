...
pub struct Handle {
    ...
    active_request: Arc<Mutex<Option<shrimply_server_client::CancellationToken>>>,
}
...
pub fn models(server_url: &str) -> Result<Vec<String>, String> {
    let models = shrimply_server_client::server_status(server_url)?
        .capabilities
        .into_iter()
    ...
    let result = (|| {
        ...
        for (index, chunk) in chunks.into_iter().enumerate() {
            ...
            *handle
            ...
            let transcription = shrimply_server_client::transcribe(
                &server_url,
                &cancellation,
                &model,
                &chunk.samples,
