...
pub fn prepare_geometry(
    ...
    viewport: PreviewViewport,
    extensions: Option<&HashMap<PreviewExtensionKey, Box<dyn Any>>>,
    ...
    let tracked_camera = item
        ...
        .and_then(|source| {
            shrimply_video::camera_reconstruction::sample(item.id, source, evaluation.local_time())
        })
        .map(|camera| TrackedCameraPreview {
...
pub fn update_text_source_size(
    ...
    source_sizes.insert(
        item.id,
        shrimply_video::text_layout::layout(
            text,
            &content,
            font_size,
            font_weight,
