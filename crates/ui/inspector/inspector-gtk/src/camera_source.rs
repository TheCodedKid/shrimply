...
use adw::prelude::*;
use gtk::glib;
use shrimply_3dgs_core::CameraSource;
...
pub(super) fn add_controls(
    ...
    let server_url =
        shrimply_editor_state::preferences::snapshot(&context.preferences).compute_server_url;
...
fn add_presentation(
    ...
    let status_row = adw::ActionRow::builder()
        .title(shrimply_components_gtk::tr!(&status.label).as_ref())
        .subtitle(shrimply_components_gtk::tr!(&status.value).as_ref())
        .build();
    ...
    let button = gtk::Button::builder()
        .label(shrimply_components_gtk::tr!(&analysis_state.label).as_ref())
...
fn update_analysis_controls(
    ...
    button.set_label(shrimply_components_gtk::tr!(&state.label).as_ref());
