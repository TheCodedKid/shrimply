...
use glam::Vec2;
use gtk::prelude::*;
use shrimply_components_core::layered::{LayeredEdit, LayeredPropertyController};
...
use shrimply_project_document::project::{Project, ResolvedTransform, Time, Transform};
...
impl Inspectable for Transform {
    ...
    fn default_action(&self, context: &InspectorContext) -> Option<Box<dyn Fn() + 'static>> {
        ...
        Some(Box::new(move || {
            ...
            shrimply_project_document::project::commit_edit(&project, RESET_TRANSFORM_COMMIT);
            drop(project);
