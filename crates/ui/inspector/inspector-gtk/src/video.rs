...
use shrimply_components_gtk::resource_pipeline::{UiSubscription, deliver};
use shrimply_components_gtk::ui::{ColorPicker, control_row, dropdown};
...
use shrimply_project_document::svg_color;
...
impl Inspectable for VideoItem {
    fn title(&self) -> &'static str {
        ...
        match self.content {
            shrimply_project_document::project::VideoItemContent::Text(_) => "Text",
            shrimply_project_document::project::VideoItemContent::Shape(_) => "Shape",
            shrimply_project_document::project::VideoItemContent::Paint(_) => "Paint",
            shrimply_project_document::project::VideoItemContent::Background(_) => "Background",
            shrimply_project_document::project::VideoItemContent::Media => "Video",
            shrimply_project_document::project::VideoItemContent::Image => "Image",
            shrimply_project_document::project::VideoItemContent::Gif => "GIF",
            shrimply_project_document::project::VideoItemContent::Svg => "SVG",
            shrimply_project_document::project::VideoItemContent::Pdf(_) => "PDF",
            shrimply_project_document::project::VideoItemContent::Manim(_) => "Manim",
            shrimply_project_document::project::VideoItemContent::Blender(_) => "Blender",
            shrimply_project_document::project::VideoItemContent::LayeredImage(_) => {
                "Layered Image"
            }
            shrimply_project_document::project::VideoItemContent::Obj(_) => "OBJ",
    ...
    fn inspect(&self, context: &InspectorContext) -> Vec<gtk::Widget> {
            ...
            super::info::ItemInfo {
                ...
                source_metadata: if matches!(
                    ...
                    shrimply_project_document::project::VideoItemContent::Media
                        | shrimply_project_document::project::VideoItemContent::Gif
                ) {
        ...
        if let Some(items) = super::generated::items(self, context) {
            ...
            match &self.content {
                shrimply_project_document::project::VideoItemContent::Paint(paint) => {
                    visual_items.extend(super::paint::items(paint));
                }
                shrimply_project_document::project::VideoItemContent::Background(background) => {
                    visual_items.push(super::background::item(background));
                }
                shrimply_project_document::project::VideoItemContent::Obj(scene) => {
                    visual_items.extend(super::scene_3d::items(scene, context));
                }
                shrimply_project_document::project::VideoItemContent::Gaussian(scene) => {
                    visual_items.extend(super::gaussian_3d::items(scene, context));
                }
                shrimply_project_document::project::VideoItemContent::Text(_)
                | shrimply_project_document::project::VideoItemContent::Shape(_)
                | shrimply_project_document::project::VideoItemContent::Media
                | shrimply_project_document::project::VideoItemContent::Image
                | shrimply_project_document::project::VideoItemContent::Gif
                | shrimply_project_document::project::VideoItemContent::Svg
                | shrimply_project_document::project::VideoItemContent::Pdf(_)
                ...
                | shrimply_project_document::project::VideoItemContent::FoldedSequence(_) => {
                    ...
                    if let shrimply_project_document::project::VideoItemContent::LayeredImage(
                        image,
                    ) = &self.content
                    {
                        visual_items.push(
                        DefaultInspectorItem::new(
                            ...
                            |context, _: LayeredImageLayers| {
                                apply_video_reset(context, "reset-layered-image-layers", |item| {
                                    if let shrimply_project_document::project::VideoItemContent::LayeredImage(image) =
                                        &mut item.content
                                    {
                                        image.layers.clear();
                                    }
        ...
        match self.source_visual_kind() {
            shrimply_visual_modifiers::VisualKind::Vector => visual_items.push(
                DefaultInspectorItem::new(
                    "skia-drawing",
                    "Skia drawing",
            ...
            shrimply_visual_modifiers::VisualKind::Raster
...
fn reset_video_stabilization(context: &InspectorContext, _: Option<VideoItem>) {
    apply_video_reset(
        ...
        |item: &mut VideoItem| {
            shrimply_visual_cuda::video_stabilization::cancel(item);
            ...
            item.mesh_flow_rows = shrimply_project_document::project::default_mesh_flow_rows();
            item.mesh_flow_columns =
                shrimply_project_document::project::default_mesh_flow_columns();
            ...
            item.mesh_flow_iterations =
                shrimply_project_document::project::default_mesh_flow_iterations();
...
fn video_stabilization_rows(item: &VideoItem, context: &InspectorContext) -> Vec<gtk::Widget> {
            ...
            update_video_item(
                ...
                move |item| {
                    ...
                    if item.stabilize_video {
                        shrimply_visual_cuda::video_stabilization::request(item);
                    }
                    true
                },
            );
    ...
    spinner.set_visible(
        item.stabilize_video && shrimply_visual_cuda::video_stabilization::is_generating(item),
    );
    ...
    let cache_row = adw::ActionRow::builder()
        ...
        .build();
    let generating = shrimply_visual_cuda::video_stabilization::is_generating(item);
    ...
    if let Some(key) = context.selected_item.clone() {
        ...
        rebuild.connect_clicked(move |_| {
            ...
            if let Some(item) = item {
                if shrimply_visual_cuda::video_stabilization::is_generating(&item) {
                    shrimply_visual_cuda::video_stabilization::cancel(&item);
                    return;
                }
                ...
                shrimply_visual_cuda::video_stabilization::rebuild(&item, source_position);
            }
        });
        ...
        let player_state = context.player_state.clone();
        let status_key = key;
        let mut was_generating = shrimply_visual_cuda::video_stabilization::is_generating(item);
        glib::timeout_add_local(Duration::from_millis(100), move || {
            ...
            spinner.set_visible(
                item.stabilize_video
                    && shrimply_visual_cuda::video_stabilization::is_generating(&item),
            );
            let generating = shrimply_visual_cuda::video_stabilization::is_generating(&item);
...
fn stabilization_status(item: &VideoItem) -> &'static str {
    if item.alpha_mask_video.is_some() {
        ...
        ""
    } else if shrimply_visual_cuda::video_stabilization::is_generating(item) {
        "Analyzing source motion…"
    } else if shrimply_visual_cuda::video_stabilization::has_failed(item) {
        "Analysis failed; use Rebuild to retry"
    } else if shrimply_visual_cuda::video_stabilization::is_ready(item) {
    ...
    shrimply_video_generation_gtk::editor(
        ...
        move |path, result| {
            ...
            let next_video_start = match project.track(&key.track()) {
                Some(shrimply_project_document::project::TrackRef::Video(track)) => track
                    .items
                    .iter()
            ...
            audio.playback_speed = shrimply_project_document::project::default_playback_speed();
            audio.repeat_strategy = shrimply_project_document::project::RepeatStrategy::Hold;
            ...
            item.playback_speed = shrimply_project_document::project::default_playback_speed();
            ...
            shrimply_project_document::project::commit_edit(&project, "generate-video");
            drop(project);
            player_state::refresh_project(
                &generated_player,
                ProjectChange {
...
fn layered_image_controls(
    ...
    let Ok(image) = shrimply_visual_cuda::load_layered_image(&value.file) else {
        return Vec::new();
    };
    let section = InspectorSection::controls();
...
pub(super) fn visual_local_time(project: &Project, key: SelectedItem, time: Time) -> Option<Time> {
    ...
    let item = project.video_item(&key)?;
    shrimply_project_document::project::generated_item_time(item, time)
}
...
fn update_svg_color_override(
    ...
    shrimply_project_document::project::commit_coalesced_edit(&project, "svg-color-override");
    drop(project);
    player_state::refresh_project(
        player_state,
        ProjectChange {
...
fn manim_controls(
    ...
    let source =
        shrimply_project_document::project::Asset::from(std::path::Path::new(&presentation.source));
