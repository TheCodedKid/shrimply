...
pub(crate) fn text_items(text: &TextItem, context: &InspectorContext) -> Vec<InspectorListItem> {
    let cards = shrimply_inspector_core::generated::text::cards(
        ...
        Some(
            &shrimply_editor_state::preferences::snapshot(&context.preferences)
...
fn default_text(context: &InspectorContext) -> TextItem {
    ...
    text.font_families = vec![
        shrimply_editor_state::preferences::snapshot(&context.preferences).default_text_font_family,
    ];
    text
}

fn default_text_for_canvas(
...
fn font_variation_control(control: &InspectorControl, context: &InspectorContext) -> gtk::Widget {
    ...
    let label = gtk::Label::builder()
        ...
        .hexpand(true)
        .tooltip_text(shrimply_components_gtk::i18n::text(&control.tooltip))
        .build();
