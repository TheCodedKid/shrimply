...
use shrimply_components_gtk::ui::switch_row;
...
impl Inspectable for AudioItem {
    ...
    fn inspect(&self, context: &InspectorContext) -> Vec<gtk::Widget> {
        ...
        spinner
            .set_visible(self.beat_detection && shrimply_audio_engine::beat::is_loading(self.id));
        ...
        if self.beat_detection {
            ...
            let mut observed_loading = shrimply_audio_engine::beat::is_loading(id);
            glib::timeout_add_local(Duration::from_millis(33), move || {
                let Some(spinner) = spinner.upgrade() else {
                    return glib::ControlFlow::Break;
                };
                let loading = shrimply_audio_engine::beat::is_loading(id);
...
fn reset_audio_output(context: &InspectorContext, output: AudioOutput) {
    ...
    *item.gain = output.gain;
    shrimply_project_document::project::commit_edit(&project, "reset-audio-output");
    drop(project);
...
fn update_audio_item_enabled(context: &InspectorContext, enabled: bool) {
    ...
    shrimply_project_document::project::commit_edit(&project, "toggle-audio-item-enabled");
    drop(project);
    player_state::refresh_project(
        &context.player_state,
        ProjectChange {
...
fn update_beat_detection(context: &InspectorContext, enabled: bool) {
    ...
    shrimply_project_document::project::commit_edit(&project, "toggle-beat-detection");
    drop(project);
    player_state::refresh_project(
        &context.player_state,
        ProjectChange {
...
impl Inspectable for PlaybackSpeed {
    ...
    fn default_action(&self, context: &InspectorContext) -> Option<Box<dyn Fn() + 'static>> {
        ...
        Some(Box::new(move || {
            ...
            shrimply_project_document::project::commit_edit(&project.borrow(), "audio-speed");
            refresh();
        }))
    }
...
fn update_audio_stream(
    ...
    shrimply_project_document::project::commit_edit(&project, "audio-stream");
    drop(project);
    player_state::refresh_project(
        player_state,
        ProjectChange {
...
fn update_audio_speed_method(
    ...
    shrimply_project_document::project::commit_edit(&project, "audio-speed-method");
    drop(project);
    player_state::refresh_project(
        player_state,
        ProjectChange {
...
fn update_audio_repeat_strategy(
    ...
    shrimply_project_document::project::commit_edit(&project, "audio-repeat-strategy");
    drop(project);
    player_state::refresh_project(
        player_state,
        ProjectChange {
