...
use shrimply_components_gtk::ui::InspectorLayeredProperty;
...
pub(crate) use shrimply_property_model::timeline_value::*;
...
fn layered_property<T: TimelineValueType>(
    ...
    let state = shrimply_inspector_core::LayeredState::from(value);
    let controller = LayeredPropertyController::default();
    controller.set_keyframes(state.keyframes);
    controller.set_expression(state.expression);
...
pub(crate) fn expression_section(
    ...
    let output_title = gtk::Label::new(Some(shrimply_components_gtk::tr!("Output").as_ref()));
