...
struct FontBrowserState {
    ...
    scrollbar: Rc<RefCell<shrimply_components_skia::slider::Lifecycle>>,
    scrollbar_drag_y: Rc<Cell<Option<f64>>>,
    hovered: Rc<Cell<Option<usize>>>,
    pressed: Rc<Cell<Option<usize>>>,
    buttons: Rc<RefCell<Vec<shrimply_components_skia::button::Button>>>,
    font_loader: Rc<RefCell<shrimply_components_skia::font_grid::FontLoader<TypefaceKey>>>,
...
fn show_font_dialog(
    ...
    let state = FontBrowserState {
        ...
        scrollbar: Rc::new(RefCell::new(
            shrimply_components_skia::slider::Lifecycle::default(),
        )),
        ...
        font_loader: Rc::new(RefCell::new(
            shrimply_components_skia::font_grid::FontLoader::new(
                worker_count,
...
impl FontBrowserState {
    fn rebuild(&self) {
        ...
        self.buttons.replace(vec![
            shrimply_components_skia::button::Button::default();
            visible.len()
        ]);
    ...
    fn update_hover(&self, x: f64, y: f64) {
        let next = shrimply_components_skia::font_grid::hit_test(
        ...
        if let Some(index) = previous.and_then(|index| buttons.get_mut(index)) {
            index.event(shrimply_components_skia::button::Event::PointerLeft);
        }
        if let Some(index) = next.and_then(|index| buttons.get_mut(index)) {
            index.event(shrimply_components_skia::button::Event::PointerEntered);
        }
...
fn connect_canvas(state: &FontBrowserState) {
    ...
    state.area.connect_render({
        ...
        move |area, _| {
            ...
            let maximum_scroll = (shrimply_components_skia::font_grid::content_height(
                width as f32,
            ...
            for index in visible {
                ...
                animating |= shrimply_components_skia::font_grid::draw_specimen(
                    painter.canvas(),
                    shrimply_components_skia::font_grid::cell_bounds(
                        width as f32,
                        state.scroll.get(),
                        index,
                    ),
                    shrimply_components_skia::font_grid::Specimen {
                        label: &family.name,
            ...
            if let Some(scrollbar) = scrollbar.scrollbar {
                shrimply_components_skia::draw_scrollbar(painter.canvas(), scrollbar);
            }
        ...
        move |_| {
            ...
            if let Some(index) = state.hovered.take() {
                ...
                if let Some(button) = buttons.get_mut(index) {
                    button.event(shrimply_components_skia::button::Event::PointerLeft);
                }
            }
        ...
        move |_, x, y| {
            if let Some(scrollbar) = current_font_scrollbar(&state) {
                ...
                if matches!(
                    ...
                    shrimply_components_skia::slider::Begin::Drag
                ) {
                    state.scroll.set(scroll as f32);
            ...
            if let Some(index) = pressed {
                let mut buttons = state.buttons.borrow_mut();
                if let Some(button) = buttons.get_mut(index) {
                    button.event(shrimply_components_skia::button::Event::Pressed);
                }
            }
            state.needs_animation.set(true);
            state.area.queue_render();
            ...
            let clicked = pressed
                .and_then(|index| {
                    state.buttons.borrow_mut().get_mut(index).map(|button| {
                        button
                            .event(shrimply_components_skia::button::Event::Released)
                            .clicked
                    })
                })
                .unwrap_or(false);
    ...
    scroll.connect_scroll({
        ...
        move |controller, _, dy| {
            ...
            let input = if controller.unit() == gtk::gdk::ScrollUnit::Wheel {
                shrimply_components_skia::slider::ScrollInput::Wheel
            } else {
                shrimply_components_skia::slider::ScrollInput::Surface
            };
...
fn current_font_scrollbar(state: &FontBrowserState) -> Option<shrimply_components_skia::Scrollbar> {
...
fn font_scrollbar(
    ...
    color: Color,
    outline_color: Color,
) -> Option<shrimply_components_skia::Scrollbar> {
    let content_length = shrimply_components_skia::font_grid::content_height(width, item_count);
    (content_length > height).then_some(shrimply_components_skia::Scrollbar {
        axis: shrimply_components_skia::Axis::Vertical,
        bounds: shrimply_components_skia::Rect::from_xywh(0.0, 0.0, width, height),
        content_length: f64::from(content_length),
        viewport_length: f64::from(height),
        value: f64::from(value),
        color,
        outline_color,
        state: shrimply_components_skia::slider::idle_state(),
    })
}
