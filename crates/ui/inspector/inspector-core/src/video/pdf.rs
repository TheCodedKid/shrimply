...
use shrimply_project_document::project::{Asset, AssetSnapshot, Transform, VideoItemContent};
...
pub fn card(item: &shrimply_project_document::project::VideoItem) -> VideoCard {
    ...
    match pages(&item.file) {
        ...
        PdfPages::Ready(pages) => {
            ...
            let page_count = if pages.len() == 1 {
                shrimply_i18n::text("1 page").into_owned()
            } else {
                shrimply_i18n::text_args("%{count} pages", &[("count", pages.len().to_string())])
            };
            section.add(
