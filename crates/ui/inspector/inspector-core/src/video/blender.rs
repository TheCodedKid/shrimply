...
impl crate::InspectorController {
    pub fn sync_blender_metadata(
        ...
        let shrimply_project_document::project::VideoItemContent::Blender(blender) =
            &mut item.content
        else {
        ...
        blender.camera = resolved.camera;
        item.source_duration = duration;
        shrimply_project_document::project::commit_edit(&project, "blender-metadata");
        drop(project);
...
pub(super) fn set_field(
    ...
    let shrimply_project_document::project::VideoItemContent::Blender(blender) = &mut item.content
    else {
        return None;
    };
