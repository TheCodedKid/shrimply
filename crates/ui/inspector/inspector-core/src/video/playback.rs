...
pub fn speed(item: &VideoItem) -> VideoCard {
    let value = shrimply_project_document::project::playback_speed_or_default(item.playback_speed);
    ...
    VideoCard::new("speed", "Speed", section).reset_fraction(
        "/playback_speed",
        shrimply_project_document::project::default_playback_speed(),
        "video-speed",
    )
}
...
pub(super) fn set_fraction(
    ...
    match path {
        "/playback_speed" => {
            let value = shrimply_project_document::project::playback_speed_or_default(value);
            if item.playback_speed == value {
                return Ok(false);
            }
            if shrimply_project_document::project::playback_speed_is_negative(item.playback_speed)
                != shrimply_project_document::project::playback_speed_is_negative(value)
            {
                item.time_offset =
                    shrimply_project_document::project::video_source_time_at(item, item.end)
                        .unwrap_or_else(|| {
                            if shrimply_project_document::project::playback_speed_is_negative(value)
                            {
...
pub fn frame_rate(item: &VideoItem) -> VideoCard {
    let mut choices = vec![(
        fraction_text(shrimply_project_document::project::native_playback_fps()),
        "Native".to_string(),
    )];
    choices.extend(
        COMMON_FRAME_RATES
        ...
        .all(|(value, _)| value != &fraction_text(item.playback_fps))
    {
        choices.push((
            ...
            shrimply_project_document::project::fraction_as_label(item.playback_fps),
        ));
    }
    let mut section = InspectorSection::default();
    section.add(
    ...
    VideoCard::new("frame-rate", "Frame rate", section).reset_fraction(
        "/playback_fps",
        shrimply_project_document::project::native_playback_fps(),
...
pub fn repeat(item: &VideoItem) -> VideoCard {
    ...
    VideoCard::new("repeat", "Repeat", section).reset(
        "/repeat_strategy",
        serde_json::to_value(shrimply_project_document::project::RepeatStrategy::default())
