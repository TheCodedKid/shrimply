...
use shrimply_project_document::project::{
    CanvasSize, ShapeItem, TextItem, VideoItem, VideoItemContent,
};
use shrimply_property_model::{Color, FontFamily, timeline_value::TimelineValue};
...
pub(crate) fn step_timeline_type(path: &str) -> Option<&'static str> {
    Some(match step_timeline(path)? {
        StepTimeline::Shape => {
            std::any::type_name::<shrimply_project_document::project::ShapeKind>()
        }
        StepTimeline::Rounding => {
            std::any::type_name::<shrimply_project_document::project::ShapeRoundingStrategy>()
        }
        StepTimeline::HorizontalAlign => {
            std::any::type_name::<shrimply_project_document::project::TextHorizontalAlign>()
        }
        StepTimeline::VerticalAlign => {
            std::any::type_name::<shrimply_project_document::project::VerticalAlign>()
        }
        StepTimeline::Direction => {
            std::any::type_name::<shrimply_project_document::project::TextDirection>()
        }
        StepTimeline::FontStyle => {
            std::any::type_name::<shrimply_project_document::project::TextFontStyle>()
        }
    })
}
