...
use shrimply_project_document::project::{ItemAddress, Time, VideoItem, VisualModifier};
use shrimply_property_model::modifier_model::ModifierModel;
                    ...
                    RasterModifierEffect::TransparentFill(value) => {
                        Some(VisualModifierBodyPresentation::TransparentFill(
                            transparent_fill::presentation(
                                ...
                                index,
                                modifier.id,
                                shrimply_visual_core::transparent_fill::analysis::status(
                                    project,
                                    address,
                                    modifier.id,
                                ),
            ...
            .then(|| {
                crate::alpha_mask::presentation(
                    ...
                    crate::alpha_mask::preview_focus(
                        item.id,
                        shrimply_project_document::project::VisualAlphaMaskTarget::Modifier(
                            modifier.id,
                        ),
                        true,
                    ),
...
pub(crate) fn visual_modifier_color<'a>(
    ...
    &'a shrimply_property_model::timeline_value::TimelineValue<shrimply_property_model::Color<u8>>,
> {
...
pub(crate) fn visual_modifier_color_by_id(
    ...
    &shrimply_property_model::timeline_value::TimelineValue<shrimply_property_model::Color<u8>>,
> {
    item.modifiers
        .iter()
...
pub(crate) fn visual_modifier_number<'a>(
    ...
    path: &str,
    id: uuid::Uuid,
) -> Option<&'a shrimply_property_model::timeline_value::TimelineValue<f32>> {
    let (modifier, field) = visual_modifier_at_path(item, path)?;
    match &modifier.effect {
        ModifierEffect::Vector(effect) => match &**effect {
...
pub(crate) fn visual_modifier_vector2<'a>(
    ...
    path: &str,
    id: uuid::Uuid,
) -> Option<&'a shrimply_property_model::timeline_value::TimelineValue<glam::Vec2>> {
    let (modifier, field) = visual_modifier_at_path(item, path)?;
    match &modifier.effect {
        ModifierEffect::Vector(effect) => match &**effect {
...
pub(crate) fn visual_modifier_vector3<'a>(
    ...
    path: &str,
    id: uuid::Uuid,
) -> Option<&'a shrimply_property_model::timeline_value::TimelineValue<glam::Vec3>> {
    let (modifier, field) = visual_modifier_at_path(item, path)?;
    match &modifier.effect {
        ModifierEffect::Scene3d(effect) => match &**effect {
...
pub(crate) fn visual_modifier_text<'a>(
    ...
    path: &str,
    id: uuid::Uuid,
) -> Option<&'a shrimply_property_model::timeline_value::TimelineValue<String>> {
    let (modifier, field) = visual_modifier_at_path(item, path)?;
...
pub(crate) fn erode_dilate_operation<'a>(
    ...
    &'a shrimply_property_model::timeline_value::TimelineValue<
...
pub(super) fn modifier_scalar_control(
    ...
    timeline: &shrimply_property_model::timeline_value::TimelineValue<f32>,
    runtime: InspectorRuntime,
    number: NumberSpec,
    rotating: bool,
) -> InspectorControl {
...
pub(super) fn modifier_vector2_control(
    path: String,
    label: impl Into<String>,
    timeline: &shrimply_property_model::timeline_value::TimelineValue<glam::Vec2>,
    runtime: InspectorRuntime,
    number: NumberSpec,
    lock: bool,
) -> InspectorControl {
...
pub(super) fn modifier_vector3_control(
    path: String,
    label: impl Into<String>,
    timeline: &shrimply_property_model::timeline_value::TimelineValue<glam::Vec3>,
    runtime: InspectorRuntime,
    number: NumberSpec,
    lock: bool,
    rotating: bool,
...
pub(super) fn modifier_color_control(
    path: String,
    label: impl Into<String>,
    timeline: &shrimply_property_model::timeline_value::TimelineValue<
        shrimply_property_model::Color<u8>,
    >,
    runtime: InspectorRuntime,
) -> InspectorControl {
    let value = timeline.value_at(runtime.local_time.unwrap_or(Time::ZERO));
...
pub(super) fn modifier_text_control(
    path: String,
    label: impl Into<String>,
    timeline: &shrimply_property_model::timeline_value::TimelineValue<String>,
    runtime: InspectorRuntime,
) -> InspectorControl {
...
impl InspectorController {
    ...
    pub fn set_visual_modifier_enabled(
        ...
        shrimply_project_document::project::commit_edit(&project, "toggle-visual-modifier");
        drop(project);
    ...
    pub fn reset_visual_modifier_effect(
        ...
        shrimply_project_document::project::commit_edit(&project, "reset-visual-modifier");
        drop(project);
        refresh(&self.player_state);
        Ok(())
    }
    ...
    pub fn remove_visual_modifier(
        ...
        drop(project);
        if cached {
            shrimply_visual_core::modifier_cache::invalidate(video_address(target)?, id)?;
        }
    ...
    pub fn add_visual_modifier(
        ...
        item.modifiers = modifiers;
        shrimply_project_document::project::commit_edit(&project, "add-visual-modifier");
        drop(project);
        refresh(&self.player_state);
        Ok(id)
    }
    ...
    pub fn paste_visual_modifiers(
        ...
        shrimply_project_document::project::commit_edit(&project, "paste-item-modifiers");
        drop(project);
        player_state::refresh_project(
            &self.player_state,
            ProjectChange {
...
fn configured_effect(
    ...
    item: &VideoItem,
    position: Time,
    audio: &shrimply_project_evaluation::FrameAudioAnalysis,
    ...
    let center = shrimply_project_evaluation::resolve_item_transform_with_audio(
        project,
        item,
        position,
        audio,
    ...
    match &mut effect {
        ModifierEffect::Vector(effect) => {
            if let VectorModifierEffect::Transform(transform) = &mut **effect {
                **transform =
                    shrimply_visual_modifiers::transform::TransformModifier::centered_at(center);
            }
        }
        ModifierEffect::Rasterize(rasterize) => {
            *rasterize = shrimply_visual_modifiers::rasterize::RasterizeModifier::new(canvas_size);
        }
        ModifierEffect::Raster(effect) => {
...
fn refresh(state: &shrimply_editor_state::player_state::SharedPlayerState) {
    player_state::refresh_project(
        state,
        ProjectChange {
            video: true,
