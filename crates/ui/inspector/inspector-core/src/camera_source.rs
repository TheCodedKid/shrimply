...
use shrimply_project_document::project::{ItemAddress, Project, VideoItemContent};
use shrimply_visual_core::camera_reconstruction::{self, AnalysisStatus};
...
pub fn presentation(
        ...
        .filter(|(_, track)| track.id != address.track_id())
    {
        ...
        source_labels.push(shrimply_i18n::text_args(
            "Visual track %{number}",
    ...
    let model_labels = model_values
        ...
        .map(|model| {
            ...
            if available.is_some_and(|available| !available.contains(model)) {
                shrimply_i18n::text_args("%{label} (Unavailable)", &[("label", label)])
            } else {
                label
            }
        })
...
pub fn tracking_models(server_url: &str) -> TrackingModels {
    ...
    let models = shrimply_compute_client::server_status(server_url).and_then(|status| {
        let models = status
            .capabilities
            .into_iter()
...
pub fn status_label(status: &AnalysisStatus) -> String {
    match status {
        AnalysisStatus::NotAnalyzed => shrimply_i18n::text("Not analyzed").into_owned(),
        AnalysisStatus::OutOfDate => shrimply_i18n::text("Out of date").into_owned(),
        AnalysisStatus::Queued => shrimply_i18n::text("Queued").into_owned(),
        AnalysisStatus::Loading => shrimply_i18n::text("Loading tracking model").into_owned(),
        AnalysisStatus::Analyzing {
            message,
            completed_frames,
            total_frames,
        } if *total_frames != 0 => shrimply_i18n::text_args(
            "%{message} %{completed}/%{total}",
            &[
                ("message", shrimply_i18n::text(message).into_owned()),
                ("completed", completed_frames.to_string()),
                ("total", total_frames.to_string()),
            ],
        ),
        AnalysisStatus::Analyzing { message, .. } => shrimply_i18n::text(message).into_owned(),
        AnalysisStatus::Cancelling => shrimply_i18n::text("Cancelling").into_owned(),
        AnalysisStatus::Cancelled => shrimply_i18n::text("Cancelled").into_owned(),
        AnalysisStatus::Ready { sample_count } => shrimply_i18n::text_args(
            "Ready (%{count} samples)",
            &[("count", sample_count.to_string())],
        ),
        AnalysisStatus::Failed { error } => format!("Failed: {error}"),
        AnalysisStatus::MissingSourceTrack => {
            shrimply_i18n::text("Source track unavailable").into_owned()
        }
        AnalysisStatus::EmptySourceTrack => {
            shrimply_i18n::text("Source track is empty").into_owned()
        }
    }
}
