...
fn font_families_control(
    base: &str,
    selected: &[shrimply_property_model::FontFamily],
) -> InspectorControl {
    InspectorControl::new(
...
fn font_axes(value: &Text3dModifier) -> Vec<crate::font_cache::FontAxis> {
    ...
    let capabilities = match family {
        shrimply_property_model::FontFamily::GoogleFonts { name } => {
            crate::font_cache::cached_capabilities(name).unwrap_or_default()
        }
        shrimply_property_model::FontFamily::Local { name } => {
            crate::font_cache::local_capabilities(name)
        }
    };
    capabilities
        .axes
...
fn number_control(
    ...
    field: &str,
    label: &'static str,
    value: &shrimply_property_model::timeline_value::TimelineValue<f32>,
    runtime: InspectorRuntime,
    spec: NumberSpec,
) -> InspectorControl {
...
fn set_font_families(value: &mut Text3dModifier, text: &str) -> Result<bool, String> {
    let mut next: Vec<shrimply_property_model::FontFamily> =
...
fn set_font_variation(value: &mut Text3dModifier, axis: &str, text: &str) -> Result<bool, String> {
    ...
    value
        .font_variations
        .push(shrimply_property_model::FontVariation {
            axis: axis.to_string(),
            value: next,
        });
    Ok(true)
