...
pub fn source_label(
    ...
    let Some(id) = id else {
        return shrimply_i18n::text("Drag onto a visual clip…").into_owned();
    };
    let Some(address) = address else {
        return shrimply_i18n::text("Missing item").into_owned();
    };
    ...
    tracks
        ...
        .unwrap_or_else(|| shrimply_i18n::text("Missing item").into_owned())
}
...
pub(super) fn is_mode(item: &shrimply_project_document::project::VideoItem, path: &str) -> bool {
...
impl InspectorController {
    ...
    pub fn clear_mask_source(
        ...
        mask.item_id = None;
        shrimply_project_document::project::commit_edit(&project, "edit-mask");
        drop(project);
