...
pub fn open(label: &str, title: &str, filter: &str) -> Option<PathBuf> {
    let initial = shrimply_components_core::file_picker::initial_folder(label)
...
pub fn save(
    ...
    let suggested = shrimply_components_core::file_picker::initial_folder(label)
...
fn selected_path(label: &str, url: QUrl) -> Option<PathBuf> {
    ...
    shrimply_components_core::file_picker::remember_file(label, &path);
    Some(path)
}
