...
use gtk::glib;
use gtk::prelude::*;
use shrimply_components_gtk::ui;
...
struct CachedEditor {
    ...
    active_generation: Rc<RefCell<Option<shrimply_compute_client::CancellationToken>>>,
    server_url: String,
    has_output: bool,
}
...
pub fn editor(
    ...
    let active_generation = Rc::new(RefCell::new(
        None::<shrimply_compute_client::CancellationToken>,
    ));
    {
    ...
    }
    {
        ...
        generate.clone().connect_clicked(move |button| {
            ...
            *active_generation.borrow_mut() = Some(cancellation.clone());
            ...
            let directory =
                shrimply_project_document::project::project_directory().join("media/video-generation");
