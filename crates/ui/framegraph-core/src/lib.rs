...
impl FrameGraphCore {
    ...
    pub fn edit_value(&self, value: f64) -> Vec<FrameGraphComponentAction> {
        self.0.borrow_mut().active_actions(|state| state.set_value(value))
    }
    ...
    pub fn edit_component_values(
        ...
        self.0.borrow_mut().set_component_values(active_component, values)
    }
    ...
    pub fn replace_active_graph(&self, graph: KeyframeGraph) {
        ...
        self.0.borrow_mut().replace_component_graph(component, graph);
    }
    ...
    pub fn set_interpolation(&self, owner_id: Uuid, interpolation: Interpolation) {
        self.0.borrow_mut().set_interpolation(owner_id, interpolation);
    }
    ...
    pub fn command(&self, command: FrameGraphCommand) -> FrameGraphInputResult {
        ...
        FrameGraphInputResult { actions, focus: true, handled: true, redraw: true }
    }

    pub fn pointer_moved(&self, x: f64, y: f64) -> FrameGraphInputResult {
        self.0.borrow_mut().pointer_moved(x, y);
        FrameGraphInputResult { redraw: true, ..Default::default() }
    }
    ...
    pub fn begin_pointer(
        ...
        let actions = self.0.borrow_mut().active_actions(|state| {
            state.begin_pointer(
                button, position.x, position.y, position.width, position.height, modifiers,
            )
        });
        FrameGraphInputResult { actions, focus: true, handled: true, redraw: true }
    }
    ...
    pub fn end_pointer(&self) -> FrameGraphInputResult {
        FrameGraphInputResult {
            actions: self.0.borrow_mut().active_actions(FrameGraphState::end_pointer),
            handled: true,
            redraw: true,
            ..Default::default()
        }
    ...
    pub fn scroll(
        ...
        FrameGraphInputResult { handled, redraw: handled, ..Default::default() }
    }
    ...
    pub fn magnify(
        ...
        self.0.borrow_mut().magnify(magnification, position.x, position.width);
        FrameGraphInputResult { handled: true, redraw: true, ..Default::default() }
    }
