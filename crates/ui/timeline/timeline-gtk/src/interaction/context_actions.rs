...
pub(super) fn show_timeline_item_context_menu(
    ...
    let contract = shrimply_timeline_skia::item_context_menu(ItemMenuContext {
        kind: match hit.kind {
...
fn show_video_track_context_menu(
    ...
    let menu = crate::native_menu::menu_model(&shrimply_timeline_skia::track_context_menu(
...
fn show_new_track_context_menu(
    ...
    let menu =
        crate::native_menu::menu_model(&shrimply_timeline_skia::empty_track_context_menu()).menu;
    let actions = gio::SimpleActionGroup::new();
    for (name, kind) in [
