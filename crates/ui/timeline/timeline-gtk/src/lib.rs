use shrimply_timeline_skia::scene::{Event as TimelineEvent, PointerButton};
...
pub use shrimply_timeline_skia::view::TimelineCursor;
...
pub struct ToolkitTimeline {
    ...
    pointer_lock: Option<shrimply_pointer_lock_wayland::WaylandPointerLock>,
...
impl ToolkitTimeline {
    ...
    pub fn activate_track_add_action(&mut self, action: TrackAddAction) -> bool {
        ...
        shrimply_timeline_skia::activate_track_add(
            ...
            action,
            settings,
        ) != shrimply_timeline_skia::TrackAddOutcome::Unchanged
    }
    ...
    fn poll_pointer_lock(&mut self) {
        if let Some((delta_x, delta_y)) = self
            .pointer_lock
            .as_mut()
            .and_then(shrimply_pointer_lock_wayland::WaylandPointerLock::poll)
        {
    ...
    pub unsafe fn begin_pointer_lock(
        ...
        surface: *mut c_void,
        seat: *mut c_void,
        software_cursor: shrimply_components_skia::cursor::SoftwareCursor,
    ) -> bool {
        if self.pointer_lock.is_some() {
            return true;
        }
        let Some(lock) = (unsafe {
            shrimply_pointer_lock_wayland::WaylandPointerLock::new(display, surface, seat)
        }) else {
            return false;
        };
        let position = self
