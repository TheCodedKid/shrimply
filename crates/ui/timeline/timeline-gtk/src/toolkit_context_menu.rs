...
impl ToolkitTimeline {
    pub fn prepare_context_menu(&mut self, x: f32, y: f32) -> usize {
        ...
        if let Some(hit) = folded_hit {
            ...
            self.context_menu =
                shrimply_timeline_skia::folded_item_context_menu(FoldedItemMenuContext {
                    groupable,
                    ungroupable,
                    folder,
        ...
        if empty_above || empty_below {
            ...
            self.reset_context_interaction();
            self.context_menu = shrimply_timeline_skia::empty_track_context_menu();
            self.context_new_track_at_top = Some(empty_above);
            return self.context_menu_entry_count();
        }
        if let Some(track_row) = track_row.filter(|row| row.root_key.is_none()) {
            self.reset_context_interaction();
            self.context_menu = shrimply_timeline_skia::folded_track_context_menu();
        ...
        if let Some(key) = key {
            ...
            self.context_menu = match key.kind {
                TrackKind::Caption => {
                    shrimply_timeline_skia::track_context_menu(TrackMenuContext::Caption)
                }
                TrackKind::Video => {
                    shrimply_timeline_skia::track_context_menu(TrackMenuContext::Video)
                }
                TrackKind::Audio => {
                    let project = self.project.borrow();
                    shrimply_timeline_skia::track_context_menu(TrackMenuContext::Audio {
    ...
    fn add_selected_item_actions(&mut self, hit: ItemKey, folder: Option<project::ItemAddress>) {
        ...
        self.context_menu = shrimply_timeline_skia::item_context_menu(ItemMenuContext {
            kind: match hit.kind {
...
pub(crate) use shrimply_timeline_skia::video_selection::prepare_selected_video_frame;
