    ...
    fn gesture(&self, input: PointerInput) -> PaintGesture {
        ...
        PaintGesture {
            ...
            eraser: self.state.eraser
                || matches!(
                    input.tool,
                    shrimply_preview_provider_skia::PointerTool::Eraser
                ),
    ...
    fn on_pointer(
        ...
        match event {
            ...
            PointerEvent::Begin(input) => {
                self.pointer = Some(input);
                if input.button != shrimply_preview_provider_skia::PointerButton::Primary {
                    return PreviewResponse::IGNORED;
                }
...
fn draw_paint_layer(
    canvas: &shrimply_preview_provider_skia::PreviewCanvas,
    renderer: PreviewPaintRenderer<'_>,
    mut draw: impl FnMut(&shrimply_preview_provider_skia::PreviewCanvas),
) {
    canvas.save();
    canvas.concat(&shrimply_math_geometry::to_skia_matrix(
        renderer.mapping.canvas_to_screen,
    ));
    shrimply_preview_provider_skia::drawing::draw_composited(
        canvas,
        renderer.opacity,
        renderer.blend_mode,
        |canvas| {
            shrimply_preview_provider_skia::drawing::draw_with_operations(
                canvas,
