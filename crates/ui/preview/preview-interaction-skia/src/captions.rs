use shrimply_components_skia::canvas::{Color, Rect, TimelinePainter, Vec2, vec2};
use shrimply_project_document::caption::markup::{self, Span};
...
fn caption_layout(
    ...
    let vertical = item.writing_direction
        != shrimply_project_document::project::CaptionWritingDirection::Horizontal;
...
fn split_for_layout(
    ...
    let text_byte = shrimply_project_document::caption::markup::plain_text_byte_at_visible_byte(
        &item.text,
        layout.elapsed_millis,
        visible_byte,
    )?;
    shrimply_project_document::caption::markup::split_at_plain_text_byte(&item.text, text_byte)?;
...
fn draw_caption_edge(
    ...
    use shrimply_project_document::project::CaptionEdgeStyle;
...
fn caption_position(
    ...
    preview_rect: Rect,
    margin: f32,
    size: shrimply_components_skia::canvas::Vec2,
    automatic_bottom: bool,
    bottom_inset: f32,
) -> Vec2 {
