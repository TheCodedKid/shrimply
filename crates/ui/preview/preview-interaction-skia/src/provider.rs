...
pub type CameraSampler =
    fn(uuid::Uuid, &shrimply_3dgs_core::TrackingCameraSource, Time) -> Option<TrackedCameraPreview>;
...
pub fn prepare_snap_scene(
    ...
    for track in tracks.iter().filter(|track| track.enabled) {
        for item in &track.items {
            ...
            if !shrimply_project_evaluation::resolve_bool(
                &item.visibility,
                &evaluation,
...
impl PreviewContext for BuildContext<'_> {
    ...
    fn extension(&self, _target: PreviewTarget, key: PreviewExtensionKey) -> Option<&dyn Any> {
        if key == shrimply_project_document::project::TRACKED_CAMERA_PREVIEW {
...
impl PreviewBuilder for BuildContext<'_> {
    fn resolve<T: TimelineExpressionValue>(&self, value: &TimelineValue<T>) -> T {
        shrimply_project_evaluation::resolve(
            value,
            self.evaluation,
...
pub fn update_text_source_size(
    ...
    let content =
        shrimply_project_evaluation::resolve_text(&text.text, evaluation, &mut expressions);
    let font_size =
        shrimply_project_evaluation::resolve_scalar(&text.font_size, evaluation, &mut expressions)
            .max(1.0);
    let font_weight = shrimply_project_evaluation::resolve_scalar(
        &text.font_weight,
        evaluation,
        &mut expressions,
    );
    let tracking =
        shrimply_project_evaluation::resolve_scalar(&text.tracking, evaluation, &mut expressions);
    let line_height = shrimply_project_evaluation::resolve_scalar(
        &text.line_height,
        evaluation,
        &mut expressions,
    )
    ...
    source_sizes.insert(
        item.id,
        shrimply_visual_core::text_layout::layout(
            text,
            &content,
            font_size,
            font_weight,
