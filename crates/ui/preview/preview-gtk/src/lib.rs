...
mod fullscreen;
mod preview_surface;
use shrimply_preview_runtime_cuda::{PreviewMedia, StepDirection};
pub use shrimply_preview_runtime_cuda::{
    playback_speed_label, playback_time_label, rendered_frame_rate_label,
};

pub use shrimply_audio_engine as audio;
pub use shrimply_components_gtk::{gl_loader, playback_shortcuts};
...
use shrimply_paint_edit_skia::{
    PAINT_PREVIEW_STATE, PaintPreviewMode as PaintMode, PaintPreviewState,
};
use shrimply_playback_performance as playback_performance;

use shrimply_preview_provider_skia::playback::{
    LoadingIndicator, STEP_REPEAT_TICK, playback_lagging,
};
...
use shrimply_paint_edit_skia::DEFAULT_PAINT_ERASER_SCALE;
...
fn stepped_paint_scale(scale: f32, larger: bool) -> f32 {
    shrimply_paint_edit_skia::step_tool_size(scale, larger)
}

fn stepped_fill_tolerance(tolerance: f32, larger: bool) -> f32 {
    shrimply_paint_edit_skia::step_fill_tolerance(tolerance, larger)
}
...
fn rebuild_paint_palette_buttons(
    ...
    *structure.borrow_mut() = Some((palette.1, palette.2.clone()));
    ...
    for (index, color_id) in palette.2.into_iter().enumerate() {
        ...
        button.set_size_request(PREVIEW_TOOLBAR_ICON_SIZE, PREVIEW_TOOLBAR_ICON_SIZE);
        button.set_tooltip_text(Some(&shrimply_components_gtk::i18n::text_args(
...
fn update_controls(
    ...
    playback_speed_label.set_tooltip_text(Some(&shrimply_components_gtk::i18n::text_args(
