use shrimply_components_skia::canvas::{Rect, Stroke, StrokeKind, Vec2, vec2};
use skia_safe::PathBuilder;

use shrimply_components_skia::{canvas::TimelinePainter, cursor};
...
pub struct KeyframeGraphDraw<'a> {
    ...
    pub frame_step: Time,
    pub scrollbar: Option<shrimply_components_skia::Scrollbar>,
    pub overscroll: Option<(shrimply_components_skia::Edge, f64)>,
    pub playhead: Time,
...
pub fn draw_keyframes(draw: KeyframeGraphDraw<'_>) {
    ...
    if let Some(scrollbar) = scrollbar {
        shrimply_components_skia::slider::draw(painter.canvas(), scrollbar);
    }
}
...
fn draw_graph_overscroll(
    ...
    width: f64,
    content_height: f64,
    edge: shrimply_components_skia::Edge,
    distance: f64,
) {
    ...
    shrimply_components_skia::draw_overshoot(
        painter.canvas(),
        shrimply_components_skia::Rect::from_xywh(
            0.0,
