...
impl Renderer {
    ...
    pub fn draw_vector(
        ...
        if self.skia.is_none() {
            ...
            let backend = unsafe {
                mtl::BackendContext::new(
                    Retained::as_ptr(&self.context.device) as mtl::Handle,
                    Retained::as_ptr(&self.context.queue) as mtl::Handle,
                )
            };
            self.skia = Some(
        ...
        let bytes = unsafe {
            std::slice::from_raw_parts_mut(output.metal().contents().as_ptr().cast(), length)
        };
