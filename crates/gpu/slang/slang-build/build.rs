...
fn main() {
    ...
    let source = env::var_os("SLANG_SOURCE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../../../external/slang")
        });
    let build = env::var_os("SLANG_BUILD_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| source.join("build"));
    let prebuilt = env::var_os("SLANG_PREBUILT").is_some_and(|value| !value.is_empty());
    let library_dir = env::var_os("SLANG_LIBRARY_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| build.join(CONFIGURATION).join("lib"));
    let include_dir = env::var_os("SLANG_INCLUDE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| source.join("include"));
