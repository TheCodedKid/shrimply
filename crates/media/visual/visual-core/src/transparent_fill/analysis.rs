...
struct Input {
    ...
    points: Vec<shrimply_visual_modifiers::transparent_fill::TransparentFillPoint>,
    tolerance: shrimply_property_model::timeline_value::TimelineValue<f32>,
    maximum_gap: u32,
}
    ...
    S: FrameSource + 'static,
{
    ...
    let spawn = thread::Builder::new()
        ...
        .spawn(move || {
            let result = source()
                .and_then(|mut source| analyze_inner(input, run_id, &cancelled, &mut source));
            let mut jobs = JOBS
                .lock()
                .expect("transparent fill analysis job lock is poisoned");
...
fn analyze_inner(
    ...
    let result = (|| {
        ...
        let expected_rgba = usize::try_from(width)
            ...
            .ok_or("transparent fill frame dimensions overflow")?;
        let total = u64::try_from(input.frames.len())
            .map_err(|_| "transparent fill frame count is too large")?;
        let item = input
            .project
            ...
            for sample in &input.frames {
                ...
                let rgba = loop {
                    match source.frame(
                        ...
                        FrameStatus::Ready(rgba) => break rgba,
                    }
                };
                if rgba.len() != expected_rgba {
                    return Err(format!(
                        "transparent fill frame has {} RGBA bytes; expected {expected_rgba}",
                        rgba.len()
                    ));
                }
                let local_time = shrimply_project_document::project::generated_item_time(
                    item,
