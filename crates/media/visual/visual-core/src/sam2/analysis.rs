...
static CANCELLATIONS: LazyLock<
    Mutex<HashMap<AnalysisTarget, (RunId, shrimply_compute_client::CancellationToken)>>,
> = LazyLock::new(|| Mutex::new(HashMap::new()));
#[derive(Default)]
struct Claims {
...
fn take_cancellation(
    target: &AnalysisTarget,
    run_id: RunId,
) -> Option<(RunId, shrimply_compute_client::CancellationToken)> {
    let mut cancellations = CANCELLATIONS
        .lock()
        .expect("SAM2 cancellation lock is poisoned");
    cancellations
...
fn analyze_clip(
    ...
    let prompt_time =
        shrimply_project_document::project::generated_item_time(item, seed.sequence_position)
            .unwrap_or(Time::ZERO);
    let request = shrimply_compute_client::Sam2AnalysisRequest::new(
        match job.modifier.model {
            shrimply_visual_modifiers::sam2::Sam2Model::Tiny => {
                shrimply_compute_client::Sam2Model::Tiny
            }
            shrimply_visual_modifiers::sam2::Sam2Model::Small => {
                shrimply_compute_client::Sam2Model::Small
            }
            shrimply_visual_modifiers::sam2::Sam2Model::BasePlus => {
                shrimply_compute_client::Sam2Model::BasePlus
            }
            shrimply_visual_modifiers::sam2::Sam2Model::Large => {
                shrimply_compute_client::Sam2Model::Large
            }
        },
        total_frames,
        job.seed_frame,
        job.modifier
            ...
            .map(|point| {
                ...
                shrimply_compute_client::Sam2Point {
                    position,
                    label: match point.label {
                        shrimply_visual_modifiers::sam2::Sam2PointLabel::Foreground => 1,
                        shrimply_visual_modifiers::sam2::Sam2PointLabel::Background => 0,
                    },
                }
            })
            .collect(),
        job.modifier
            .box_prompt
            .map(|box_prompt| shrimply_compute_client::Sam2Box {
    ...
    shrimply_compute_client::write_sam2_archive_header(archive.as_file_mut(), &request)?;
    ...
    for (completed, frame) in job.frames.iter().enumerate() {
        ...
        shrimply_compute_client::write_sam2_archive_frame(archive.as_file_mut(), &jpeg)?;
    ...
    let cancellation = shrimply_compute_client::CancellationToken::new(&job.server_url)?;
    ...
    shrimply_compute_client::analyze_sam2(
        &job.server_url,
        &cancellation,
        archive.path(),
        |event| {
            ...
            match event {
                shrimply_compute_client::Sam2Event::Queued { position } => update_progress(
                    job,
                    &shrimply_compute_client::queued_status(position),
                    total_frames,
                    progress_total,
                ),
                shrimply_compute_client::Sam2Event::Progress {
                    message,
                    completed_frames,
                    ..
                } => update_progress(
                ...
                shrimply_compute_client::Sam2Event::Mask { frame_index, mask } => {
                ...
                shrimply_compute_client::Sam2Event::Result { completed_frames } => {
                    result_frames = Some(completed_frames);
                    true
                }
                shrimply_compute_client::Sam2Event::Error { code, message } => {
