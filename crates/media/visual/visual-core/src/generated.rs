...
use shrimply_drawing_skia::{CanvasOperation, draw_with_operations};
...
use shrimply_visual_modifiers::text_mask::{TextMaskDirection, TextMaskPartialMode};
...
pub struct GeneratedTransition {
    pub kind: shrimply_project_document::project::VisualTransitionKind,
    pub side: shrimply_project_document::project::TransitionSide,
    pub progress: f32,
    pub interpolation: shrimply_project_document::project::Interpolation,
    pub ordering: shrimply_project_document::project::WriteOrdering,
    pub drawing_stroke_overlap: f32,
    pub drawing_stroke_length_weight: f32,
    pub drawing_fill_mode: shrimply_project_document::project::DrawingFillMode,
    pub morph_unit: shrimply_project_document::project::MorphUnit,
...
use shrimply_project_document::project::{Time, TransitionSide, VideoItem, VisualTransitionKind};
...
pub fn render_canvas(
    ...
    item.modifiers
        ...
        .find_map(|modifier| match &modifier.effect {
            shrimply_visual_modifiers::ModifierEffect::Rasterize(effect) => Some(effect),
            _ => None,
        })
        .map(|effect| {
            let size =
                shrimply_project_evaluation::resolve_vec2(effect.size(), evaluation, expressions);
