...
pub fn prepare(
    ...
    let stroke_transform = shrimply_project_evaluation::resolve_transform(
    ...
    let palette: Vec<_> = paint
        ...
        .map(|entry| {
            let color =
                shrimply_project_evaluation::resolve_color(&entry.color, &evaluation, expressions);
    ...
    let drawing = shrimply_project_evaluation::resolve_paint_drawing(
        &paint.drawing,
        &evaluation,
        expressions,
        paint.palette.len(),
    ...
    let path_offsets: Vec<_> = item
        ...
        .filter_map(|modifier| {
            ...
            let shrimply_visual_modifiers::VectorModifierEffect::PathOffset(effect) = &**effect
            else {
                return None;
            };
            Some(shrimply_project_evaluation::resolve_path_offset_modifier(
                effect,
                &evaluation,
                expressions,
            ))
