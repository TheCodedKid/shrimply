...
use shrimply_gpu_cuda_memory::{ResourceKey, global as gpu_memory};
use shrimply_layered_image_loader::LayeredImage;
use shrimply_project_document::project::{CanvasSize, VideoItem, VideoItemContent};
use shrimply_project_evaluation::{TransformExpressionCache, VisualEvaluation};
use uuid::Uuid;
...
impl LayeredImageAsset {
    ...
    pub(crate) fn preload(&mut self) -> Result<(), String> {
        ...
        rayon::spawn(move || {
            let _measurement = shrimply_profiling::measure("Layered image / Preload");
            let _ = reply.send(prepare_layered_image(&file));
        });
        self.pending = Some(response);
        shrimply_profiling::increment("Layered image / Preloads submitted");
        Ok(())
    }
        ...
        if let Some(pending) = self.pending.take() {
            ...
            match result {
                Ok(LoadedLayeredImage {
                    ...
                    shrimply_profiling::increment("Layered image / Preloads consumed");
    ...
    fn composite(
        ...
        let (loaded, refreshing) = {
            let _measurement = shrimply_profiling::measure("Layered image / Reload check");
            self.reload(
        ...
        let prepared = {
            let _measurement = shrimply_profiling::measure("Layered image / Resolve visibility");
            shrimply_visual_core::layered_image::prepare(
                Arc::clone(&document),
                self.snapshot
                    .as_ref()
        ...
        if let Some(layer) = gpu_memory().get_resource::<VisualFrame>(&composite_key)? {
            shrimply_profiling::increment("Layered image GPU cache / Composite hit");
            compositor.prepare_host_backed_frame(&layer, "cached layered image composite")?;
            return Ok((Some(Rc::new((*layer).clone())), refreshing));
        }
        shrimply_profiling::increment("Layered image GPU cache / Composite miss");
        ...
        for (index, source) in prepared.document.layers.iter().enumerate() {
            ...
            if let Some(layer) = gpu_memory().get_resource::<VisualFrame>(&key)? {
                shrimply_profiling::increment("Layered image GPU cache / Source hit");
                compositor.prepare_host_backed_frame(&layer, "cached layered image source")?;
                source_layers.push(Rc::new((*layer).clone()));
                continue;
            }
            shrimply_profiling::increment("Layered image GPU cache / Source miss");
        ...
        let mut layer = {
            let _measurement = shrimply_profiling::measure("Layered image / Composite submission");
...
fn prepare_layered_image(file: &Asset) -> Result<LoadedLayeredImage, String> {
    ...
    let document = if let Some(document) = gpu_memory().get_resource::<Arc<LayeredImage>>(&key)? {
        shrimply_profiling::increment("Layered image source residency / Hit");
        Arc::clone(&document)
    } else {
        shrimply_profiling::increment("Layered image source residency / Miss");
        shrimply_layered_image_loader::load(file)?
    };
...
enum LayeredFrame<'a> {
    Source(usize),
    Composite(&'a shrimply_visual_core::layered_image::Prepared),
}
