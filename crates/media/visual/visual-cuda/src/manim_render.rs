...
use shrimply_manim_bridge::Progress;
...
impl VisualElement for ManimElement {
    ...
    fn manim_status(&self, error: Option<String>) -> Option<shrimply_manim_state::Update> {
    ...
    fn draw(
        ...
        if shrimply_project_document::project::generated_source_time_at(
            request.item,
            request.position,
        )
        .is_none()
        ...
        if self.rendered_frame != Some(compiled.frame_index) {
            let _measurement = shrimply_profiling::measure("Manim / WGPU");
