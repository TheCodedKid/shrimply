...
use shrimply_project_document::project::{CanvasSize, VideoItem};
use shrimply_project_evaluation::TransformExpressionCache;
pub use shrimply_visual_core::shape::decoration_outset;
use std::rc::Rc;
use uuid::Uuid;
...
impl VisualElement for ShapeElement {
    ...
    fn draw(
        ...
        let Some(local_time) =
            shrimply_project_document::project::generated_item_time(request.item, request.position)
        else {
            return Ok(VisualRender::Empty);
        };
        let shrimply_project_document::project::VideoItemContent::Shape(_) = &request.item.content
        else {
            return Err("shape renderer received a non-shape visual".to_string());
        };
        let evaluation =
            shrimply_project_evaluation::VisualEvaluation::for_item_at_local_time_with_audio(
                request.project,
                request.item,
                request.position,
                local_time,
        ...
        let prepared = shrimply_visual_core::shape::prepare(
...
struct DeferredShapeFrame(shrimply_visual_core::shape::PreparedShape);
