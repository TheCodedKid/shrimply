...
use shrimply_gpu_cuda::LaunchConfig;
use shrimply_project_document::project::{VisualClipTransition, VisualTransition};
...
pub(crate) fn apply(
    ...
    match shrimply_visual_core::transition::raster_plan(transition, visibility, center) {
        Some(shrimply_visual_core::transition::RasterTransition::Origami(effect)) => {
        ...
        Some(shrimply_visual_core::transition::RasterTransition::Pixel(effect)) => {
...
pub(crate) fn apply_clip_mask(
    ...
    if let Some(effect) = shrimply_visual_core::transition::clip_mask(transition, progress) {
        visual.push_pixel(Box::new(effect));
    }
}
