...
use shrimply_gpu_cuda::{CudaContext, CudaEvent, CudaStream, DeviceCopy, LaunchConfig, sys};
use shrimply_gpu_cuda_memory::GpuBuffer as DeviceBuffer;
use shrimply_project_document::project::{CanvasSize, Color};
...
const KERNEL_MODULES: &[(&str, &[u8])] = &[
    ("preview", shrimply_render_kernels_cuda::PREVIEW),
    ("export", shrimply_render_kernels_cuda::EXPORT),
    ("background", shrimply_render_kernels_cuda::BACKGROUND),
    ("Anime4K", shrimply_render_kernels_cuda::ANIME4K),
    ("modifiers", shrimply_render_kernels_cuda::MODIFIERS),
    (
        "blur modifiers",
        shrimply_render_kernels_cuda::MODIFIERS_BLUR,
    ),
    (
        "geometry modifiers",
        shrimply_render_kernels_cuda::MODIFIERS_GEOMETRY,
    ),
    (
        "matte modifiers",
        shrimply_render_kernels_cuda::MODIFIERS_MATTE,
    ),
    ("stabilization", shrimply_render_kernels_cuda::STABILIZATION),
    ("mesh flow", shrimply_render_kernels_cuda::MESH_FLOW),
];
...
pub fn preflight() -> Result<(), String> {
    ...
    tracing::info!(
        image_format = shrimply_render_kernels_cuda::IMAGE_FORMAT,
        image_target = shrimply_render_kernels_cuda::IMAGE_TARGET,
...
pub struct CudaVideoCompositor {
    ...
    anime4k: shrimply_anime4k_cuda::Workspace,
...
impl CudaVideoCompositor {
    pub fn new() -> Result<Self, String> {
        ...
        let anime4k = shrimply_anime4k_cuda::Workspace::new(context.clone())?;
    ...
    pub(crate) fn estimate_optical_flow(
    ...
    ) -> Result<shrimply_visual_core::raster_morph::OpticalFlowField, String> {
        ...
        shrimply_visual_core::raster_morph::OpticalFlowField::new(
            grid_size,
    ...
    pub(crate) fn render_vector_morph(
        ...
        drawing_strategy: shrimply_project_document::project::SkiaDrawingStrategy,
    ...
    fn render_frame(
        ...
        shrimply_gpu_cuda_memory::global().begin_frame();
        self.release_after_reported_gpu_oom()?;
    ...
    fn render_frame_once(
        ...
        let mut prepared = {
            let _measurement = shrimply_profiling::measure("CUDA compositor / Prepare layers");
            layers::prepare(
                self.context.cu_ctx(),
                &self.stream,
                canvas_size,
        ...
        let mut buffer = {
            let _measurement = shrimply_profiling::measure("CUDA compositor / Acquire output");
        ...
        let _decode_waits = {
            let _measurement =
                shrimply_profiling::measure("CUDA compositor / Wait for decoded frames");
        ...
        let _anime4k_buffers = {
            let _measurement = shrimply_profiling::measure("CUDA compositor / Anime4K");
        ...
        let params = {
            let _measurement = shrimply_profiling::measure("CUDA compositor / Upload layer params");
        ...
        {
            let _measurement = shrimply_profiling::measure("CUDA compositor / Launch");
        ...
        if !export {
            let _measurement = shrimply_profiling::measure("CUDA compositor / Synchronize");
            self.stream
                .synchronize()
    ...
    pub(crate) fn render_generated_visual(
        ...
        evaluation: &shrimply_project_evaluation::VisualEvaluation,
    ...
    pub(crate) fn render_scene_3d(
        ...
        width: u32,
        height: u32,
        params: &shrimply_render_3d_core::SceneRenderParams,
    ...
    fn allocate_buffer<T: DeviceCopy>(
        ...
        match shrimply_gpu_cuda_memory::global().allocate_buffer(
            &self.stream,
            length,
            ...
            Err(error) => {
                ...
                shrimply_gpu_cuda_memory::global()
                    .allocate_buffer(
                        &self.stream,
                        length,
                        shrimply_gpu_cuda_memory::AllocationClass::Transient,
    ...
    fn relieve_gpu_pressure(
        ...
        let migrated_bytes = shrimply_gpu_cuda_memory::global().relieve_vram_pressure(
            &self.context,
            &self.stream,
            requested_bytes,
            false,
        ...
        if free >= reserve.saturating_add(requested_bytes) {
            let telemetry = shrimply_gpu_cuda_memory::global().telemetry();
            tracing::warn!(
        ...
        shrimply_profiling::increment("GPU pressure / Events");
        shrimply_profiling::add_to_counter("GPU pressure / Bytes recovered", recovered_bytes);
        if relief_level == "last-resort external resources" {
            let last_resort_recovered = last_resort_before
                .zip(after)
                .map_or(0, |((before, _), (after, _))| after.saturating_sub(before));
            shrimply_gpu_cuda_memory::global().note_last_resort_cleanup(last_resort_recovered);
        }
        self.record_gpu_memory_usage();
        let telemetry = shrimply_gpu_cuda_memory::global().telemetry();
        tracing::warn!(
    ...
    fn spill_stale_frames_for_display_memory(&mut self) -> Result<(), String> {
        ...
        if free < total / DISPLAY_GPU_MEMORY_RESERVE_DIVISOR {
            shrimply_gpu_cuda_memory::global().relieve_vram_pressure(
                &self.context,
                &self.stream,
                0,
                true,
    ...
    fn record_gpu_memory_usage(&self) {
        if let Ok((free, total)) = self.gpu_memory_info() {
            shrimply_profiling::set_counter("CUDA memory / Free bytes", free);
            shrimply_profiling::set_counter("CUDA memory / Used bytes", total.saturating_sub(free));
            shrimply_profiling::set_counter("CUDA memory / Total bytes", total);
        }
        let (frames, bytes) = gpu_allocation_stats();
        shrimply_profiling::set_counter("Visual frame / GPU frames retained", frames);
        shrimply_profiling::set_counter("Visual frame / GPU bytes retained", bytes);
    }
    ...
    pub(crate) fn retain_host_backed_frame(
        ...
        self.fail_if_superseded()?;
        if shrimply_gpu_cuda_memory::global()
            .telemetry()
            .host_budget_bytes
            == 0
        {
        ...
        match render(
            ...
            Err(error) if is_gpu_oom(&error) && relief_level == 1 => {
                ...
                let recovered_bytes = before
                    .zip(after)
                    .map_or(0, |((before, _), (after, _))| after.saturating_sub(before));
                shrimply_gpu_cuda_memory::global().note_last_resort_cleanup(recovered_bytes);
                let telemetry = shrimply_gpu_cuda_memory::global().telemetry();
                let (free, total) = after.unwrap_or_default();
                relief_level = 2;
                tracing::warn!(
