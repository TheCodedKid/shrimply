...
use shrimply_gpu_cuda::cuda_launch;
use shrimply_gpu_cuda::{
...
impl Workspace {
    pub fn new(context: Arc<CudaContext>) -> Result<Self, String> {
        let module = context
            .load_module_from_image(shrimply_render_kernels_cuda::ANIME4K)
    ...
    pub fn upscale(
        ...
        let mut pixels = shrimply_gpu_cuda_memory::global().allocate_buffer::<u32>(
            stream,
            pixel_count,
    ...
    fn convert_source(&self, stream: &Arc<CudaStream>, source: Source) -> Result<Image, String> {
        ...
        let mut pixels = shrimply_gpu_cuda_memory::global().allocate_buffer::<[f32; 4]>(
            stream,
            count,
            shrimply_gpu_cuda_memory::AllocationClass::Transient,
            "Anime4K input",
    ...
    fn run_model(
        ...
        for stage in &model.stages {
            ...
            let mut output = shrimply_gpu_cuda_memory::global().allocate_buffer::<[f32; 4]>(
                stream,
                count,
                shrimply_gpu_cuda_memory::AllocationClass::Transient,
...
impl ParsedStage {
    fn upload(self, stream: &Arc<CudaStream>) -> Result<Stage, String> {
        let mut terms = shrimply_gpu_cuda_memory::global().allocate_buffer(
            stream,
            self.terms.len(),
            shrimply_gpu_cuda_memory::AllocationClass::Persistent,
