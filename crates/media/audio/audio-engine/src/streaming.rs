...
fn mix_track_range(
    project: &Project,
    track: &shrimply_project_document::project::AudioTrack,
    sessions: &mut HashMap<AudioSourceKey, AudioRenderSession>,
    mixed: &mut [f32],
    start_frame: u64,
    end_frame: u64,
    ...
    for (item_index, item) in track.items.iter().enumerate() {
        ...
        let session = match sessions.entry(key) {
            Entry::Occupied(entry) => {
                shrimply_profiling::increment("Volume decoder cache / Hit");
                entry.into_mut()
            }
            Entry::Vacant(entry) => {
                shrimply_profiling::increment("Volume decoder cache / Miss");
                let _measurement = shrimply_profiling::measure("Volume sampling / Open decoder");
        ...
        let rendered = {
            let _measurement = shrimply_profiling::measure("Volume sampling / Decode item");
...
impl AudioDecoderSession {
    ...
    fn decode_processed_item_frames(
        ...
        if discontinuous {
            shrimply_profiling::increment("Volume sampling / Discontinuity");
            let next_effects = {
                let _measurement =
                    shrimply_profiling::measure("Volume sampling / Reinitialize effects");
                super::effects::Processor::new(item, self.sample_rate)?
            };
            self.effects = next_effects;
            self.modifier_hash = modifier_hash;
        } else {
            shrimply_profiling::increment("Volume sampling / Continuous");
        }
        let latency = self.effects.latency_frames();
        let item_start = item.start.as_sample_frame(self.sample_rate);
        let warmup = if discontinuous {
        ...
        let input_frames = frame_count
            ...
            .saturating_add(if discontinuous { latency } else { 0 });
        let mut samples = {
            let _measurement = shrimply_profiling::measure("Volume sampling / Decode samples");
            self.decode_item_frames_padded(item, input_start, input_frames)?
        };
        let local_start =
            time_from_sample_frame(input_start, self.sample_rate).saturating_sub(item.start);
        {
            let _measurement = shrimply_profiling::measure("Volume sampling / Apply effects");
            self.effects.process(&mut samples, item, local_start)?;
        }
        apply_transition_gain(&mut samples, item, input_start, self.sample_rate);
        let discard = warmup.saturating_add(if discontinuous { latency } else { 0 });
    ...
    fn seek(&mut self, frame: u64) -> Result<(), String> {
        shrimply_profiling::increment("Volume decoder / Seek");
        let _measurement = shrimply_profiling::measure("Volume sampling / Seek");
    ...
    fn decode_until(&mut self, end_frame: u64) -> Result<(), String> {
        let _measurement = shrimply_profiling::measure("Audio decode / Read packets");
    ...
    fn receive_available(&mut self) -> Result<usize, String> {
        let _measurement = shrimply_profiling::measure("Audio decode / Receive frames");
...
fn tempo_adjust_samples(
    ...
    let _measurement = shrimply_profiling::measure("Audio decode / Preserve pitch");
    let input_frames = samples.len() / CHANNELS;
    if input_frames == 0 || output_frames == 0 {
