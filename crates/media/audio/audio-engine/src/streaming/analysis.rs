...
impl VolumePeakCache {
    ...
    fn get_or_request(&self, revision: u64, frame_index: usize, indices: &[usize]) -> Option<f32> {
        ...
        {
            shrimply_profiling::increment("Volume peak cache / Hit");
            return Some(value);
        }

        shrimply_profiling::increment("Volume peak cache / Miss");
...
impl FrameVolumeSampler {
    ...
    pub fn sample(
        ...
        let _measurement = shrimply_profiling::measure("Volume sampling / Total");
        ...
        let mixer = shrimply_math_media::FrameVolumeMixer::resolving(track_count, move |indices| {
            ...
            let _measurement = shrimply_profiling::measure("Volume sampling / Mix tracks");
            let value = resolver
...
fn cache_project_volume_peaks(
    ...
    let _measurement = shrimply_profiling::measure("Volume peak cache / Build timeline");
