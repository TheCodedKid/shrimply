...
use shrimply_project_document::project::{Time, fraction_numerator};
...
impl TransformExpressionCache {
    ...
    fn entry(&mut self, key: ExpressionCacheKey) -> Option<&mut CachedExpression> {
        if !self.entries.contains_key(&key) {
            shrimply_profiling::increment("Expression cache / Miss");
            let _measurement = shrimply_profiling::measure("Expression / Compile");
            ...
            shrimply_profiling::increment("Expression cache / Hit");
        }
...
impl CachedExpression {
    fn eval_with<T>(
        ...
        let _measurement = shrimply_profiling::measure("Expression / Evaluate");
