...
use shrimply_project_document::project::{Time, default_playback_speed, scaled_time_delta};
...
pub fn step_playback_speed_forward(state: &SharedPlayerState) {
    ...
    let next = match shrimply_project_document::project::fraction_numerator(snapshot.playback_speed)
    {
        value if value < 2 => 2,
        value if value < 4 => 4,
        _ => 8,
...
fn update(state: &SharedPlayerState, change: impl FnOnce(&mut PlayerState) -> Option<PlayerEvent>) {
    ...
    .entered();
    let _measurement = shrimply_profiling::measure("Player / Dispatch");
